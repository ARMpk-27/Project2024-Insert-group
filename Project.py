        ...
        with open(file_name, 'rb') as f:
            while True:
                ...
                if unpacked_data[0] == disaster_id:
                    print("พบข้อมูลที่ต้องการอัปเดต")

                    
                    update_field = input(
                        "เลือกฟิลด์ที่ต้องการอัปเดต: 1)ประเภทภัยพิบัติ 2)สถานที่ 3)จำนวนอาสาสมัคร \n 4)ค่าวัดความรุนแรง 5)จำนวนผู้บาดเจ็บ 6)จำนวนผู้เสียชีวิต 7)วันที่\n ใส่เลขในหัวข้อที่ต้องการแก้ไข :")

                    new_record = list(unpacked_data)  

                    if "1" in update_field:
                        new_record[1] = select_disaster_type()

                    if "2" in update_field:
                        disaster_location = input("สถานที่ภัยพิบัติ: ").strip()
                        if disaster_location:
                            new_record[2] = format_string(disaster_location, 30)
                        else:
                            print("สถานที่ภัยพิบัติไม่สามารถว่างได้")

                    if "3" in update_field:
                        try:
                            new_record[3] = int(input("จำนวนอาสาสมัคร: "))
                        except ValueError:
                            print("ข้อมูลไม่ถูกต้องสำหรับจำนวนอาสาสมัคร")

                    if "4" in update_field:
                        try:
                            new_record[4] = float(input("ค่าวัดความรุนแรง: "))
                        except ValueError:
                            print("ข้อมูลไม่ถูกต้องสำหรับค่าวัดความรุนแรง")

                    if "5" in update_field:
                        try:
                            new_record[5] = int(input("จำนวนผู้บาดเจ็บ: "))
                        except ValueError:
                            print("ข้อมูลไม่ถูกต้องสำหรับจำนวนผู้บาดเจ็บ")

                    if "6" in update_field:
                        try:
                            new_record[6] = int(input("จำนวนผู้เสียชีวิต: "))
                        except ValueError:
                            print("ข้อมูลไม่ถูกต้องสำหรับจำนวนผู้เสียชีวิต")

                    if "7" in update_field:
                        timestamp = input("กรุณาใส่วันที่ (วัน/เดือน/ปี): ").strip()
                        if not timestamp:
                            timestamp = time.strftime("%d/%m/%Y")
                        new_record[7] = format_string(timestamp, 20)

                    records.append(tuple(new_record))  
                    updated = True
                else:
                    records.append(unpacked_data)
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            generate_report(file_name, report_file)  
